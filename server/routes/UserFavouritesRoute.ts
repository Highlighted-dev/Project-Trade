...
router.post('/', jsonParser, async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ status: 'ok', message: 'Added to favourites!' });
  } catch (err) {
...
router.delete('/', jsonParser, async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ status: 'ok', message: 'Removed from favourites!' });
  } catch (err) {
    return errorHandler(err, res);
  }
});
...
router.get('/check/:user_id/:product_id', jsonParser, async (req: Request, res: Response) => {
  try {
...
router.get('/:user_id?', async (req: Request, res: Response) => {
  ...
  if (!user_id) {
    const user_favourites = await userFavouritesModel.find().distinct('product_id');
    return res.status(200).json({ status: 'ok', data: user_favourites });
  }
  try {
    const user_favourites = await userFavouritesModel.find({
      user_id,
    });
    return res.status(200).json({ status: 'ok', data: user_favourites });
  } catch (err) {
