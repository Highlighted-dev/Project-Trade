...
from ..items import (
    AmazonItemThumbImages,
    AmazonItemDetails,
    AmazonItemTechnicalDetails,
    AmazonItemAbout,
)
        ...
        try:
            if self.checkForCaptcha(response):
                ...
                product_images = response.xpath(
                    '//div[@id="altImages"]//li[@class="a-spacing-small item imageThumbnail a-declarative"]//span[@class="a-button-text"]//img/@src'
                ).extract()
                product_details_name = response.xpath(
                    '//div[@id="productOverview_feature_div"]//table[@class="a-normal a-spacing-micro"]//td[@class="a-span3"]//span/text()'
                ).extract()
                product_details = response.xpath(
                    '//div[@id="productOverview_feature_div"]//table[@class="a-normal a-spacing-micro"]//td[@class="a-span9"]//span/text()'
                ).extract()
                product_technical_details_name = response.xpath(
                    '//div[@id="prodDetails"]//table[@id="productDetails_techSpec_section_1"]//th/text()'
                ).extract()
                product_technical_details = response.xpath(
                    '//div[@id="prodDetails"]//table[@id="productDetails_techSpec_section_1"]//td/text()'
                ).extract()

                for image in product_images:
                    images["product_id"] = product_id
                    images["product_thumb_image"] = image
                    self.db[GlobalVariables.mongo_column_thumb_images].replace_one(
                        {
                            "product_id": images["product_id"],
                            "product_thumb_image": images["product_thumb_image"],
                        },
                        images,
                        upsert=True,
                    )
                ...
                if len(product_details) < 1:
                    product_abouts = response.xpath(
                        '//div[@id="feature-bullets"]//ul[@class="a-unordered-list a-vertical a-spacing-mini"]//li[not(@id) and not(@class)]//span[@class="a-list-item"]/text()'
                    ).extract()
                    for about in product_abouts:
                        abouts["product_id"] = product_id
                        abouts["product_about"] = about
                        self.db[GlobalVariables.mongo_column_about].replace_one(
                            {
                                "product_id": abouts["product_id"],
                                "product_about": abouts["product_about"],
                            },
                            abouts,
                            upsert=True,
                        )
                    ...
                    for pdn, pd in zip(product_details_name, product_details):
                        ...
                        self.db[GlobalVariables.mongo_column_details].replace_one(
                            {
                                "product_id": details["product_id"],
                                "product_detail_name": details["product_detail_name"],
                            },
                            details,
                            upsert=True,
                        )
                ...
                if len(product_technical_details) < 1:
                    ...
                    for ptd, ptdn in zip(
                        product_technical_details, product_technical_details_name
                    ):
                        # ptd = product_techinal_detail | ptdn = product_technial_detail_name
                        technical_details["product_id"] = product_id
                        technical_details["product_technical_detail_name"] = "".join(
                            ptdn
                        ).strip()
                        ...
                        technical_details["product_technical_detail"] = "".join(
                            ptd
                        ).strip()
                        self.db[
                            GlobalVariables.mongo_column_technical_details
                        ].replace_one(
                            {
                                "product_id": technical_details["product_id"],
                                "product_technical_detail_name": technical_details[
                                    "product_technical_detail_name"
                                ],
                            },
                            technical_details,
                            upsert=True,
