...
class AmazonReviewsSpider(scrapy.Spider):
    ...
    def checkForReviewsFromOtherCountries(self, response):
        reviews_from_other_country = response.xpath(
            '//div[@class="a-section a-spacing-none reviews-content a-size-base"]//h3[@data-hook="dp-global-reviews-header"]/text()'
        ).extract()
    ...
    def get_reviews(self, response, product_id):
        ...
        for review in reviews:
            rating = review.xpath(
                './/div[@class="a-row a-spacing-none"]//span[@class="a-icon-alt"]/text()'
            ).extract_first()
            rating_id = review.xpath("./@id").extract_first()
            date = review.xpath(
                './/span[@class="a-size-base a-color-secondary review-date"]/text()'
            ).extract_first()
    ...
    def closed(self, reason):
        path_to_json = (
            str(Path(__file__).parents[2]) + f"/AmazonReviews.json"
        ).replace(os.sep, "/")
        ...
        try:
            ...
            average_newest_review - average_oldest_review + current_date - average_oldest_review = number_of_days_between_reviews
            Formula: 20 / number_of_days_beetween_dates * 10 = sales_per_day
            Ex. (07/07/2022 - 07/05/2021) + (09/07/2022 - 07/05/2022) = 61 + 63 = 124 days
            20 / 124 * 10 = 1.61 sales per day
            """
            product_ids = set()
            ...
            for product_id in product_ids:
                sorted_reviews_by_date = sorted(
                    [item for item in file_data if item["product_id"] == product_id],
                    key=lambda x: x["product_rating_date"],
                )
                newest_reviews = sorted_reviews_by_date[
                    len(sorted_reviews_by_date) // 2 :
                ]
                oldest_reviews = sorted_reviews_by_date[
                    : len(sorted_reviews_by_date) // 2
                ]
                ...
                def calculate_average_date_between_reviews(reviews):
                    try:
                        ...
                        date_objects = [
                            datetime.strptime(review["product_rating_date"], "%Y-%m-%d")
                            for review in reviews
                        ]

                        # Calculate average timestamp
                        avg_timestamp = sum(
                            date.timestamp() for date in date_objects
                        ) / len(date_objects)
                        ...
                        logging.error(
                            "Couldn't estimate sales for product with id: " + product_id
                        )
                        return None

                newest_review = calculate_average_date_between_reviews(newest_reviews)
                oldest_review = calculate_average_date_between_reviews(oldest_reviews)
                ...
                difference_between_days = (
                    newest_review - oldest_review + current_date.date() - oldest_review
                ).days
                sales_per_day = round(((20 / difference_between_days) * 10), 2)
                logging.info(
                    "Estimated sales for product with id: "
                    + product_id
                    + " is: "
                    + str(sales_per_day)
                )
                sales_column = self.db[GlobalVariables.mongo_column_sales]
                sales_column.replace_one(
                    {
                        "product_id": product_id,
                        "product_sales_date": current_date.strftime("%Y-%m-%d"),
                    },
                    {
                        "product_id": product_id,
